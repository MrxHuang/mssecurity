...
class UserRoleController:
    ...
    def create(user_id, role_id, data=None):
        ...
        try:
            ...
            # Parse dates - support both ISO format and custom format
            try:
                # Try ISO format first (from frontend)
                startAt = datetime.fromisoformat(data.get('startAt').replace('Z', '+00:00'))
            except:
            ...
            # endAt is optional
            endAt = None
            if 'endAt' in data and data.get('endAt'):
                try:
                    endAt = datetime.fromisoformat(data.get('endAt').replace('Z', '+00:00'))
                except:
    ...
    def update(user_role_id, data):
        ...
        try:
            ...
            # Update startAt if provided
            if 'startAt' in data and data.get('startAt'):
                try:
                    user_role.startAt = datetime.fromisoformat(data.get('startAt').replace('Z', '+00:00'))
                except:
                    user_role.startAt = datetime.strptime(data.get('startAt'), "%Y-%m-%d %H:%M:%S")
            
            # Update endAt if provided (can be None to clear it)
            if 'endAt' in data:
                if data.get('endAt'):
                    try:
                        user_role.endAt = datetime.fromisoformat(data.get('endAt').replace('Z', '+00:00'))
                    except:
                        user_role.endAt = datetime.strptime(data.get('endAt'), "%Y-%m-%d %H:%M:%S")
                else:
                    user_role.endAt = None
